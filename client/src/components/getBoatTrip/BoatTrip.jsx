...
import { FaCloudSun, FaTimes } from 'react-icons/fa'; 
...
const BoatTrip = () => {
  ...
  const fetchWeatherForecast = () => {
    if (userLocation.latitude && userLocation.longitude) {
      const API_KEY = '3f8265e2fd6333167853a1058533336f'; 
  ...
  const downloadPageAsPDF = () => {
    ...
    img.onload = function () {
      ...
      doc.setFontSize(18);
      doc.text(80, 30, "Fish Mart Fish Company");
  ...
  // ✅ Fixed sorting by Trip ID (ascending or descending)
  sortedBoatTrips = sortedBoatTrips.sort((a, b) =>
    sortByProfit ? b.tripID - a.tripID : a.tripID - b.tripID
  );
  ...
  return (
    <div className="boatboattriptable">
      ...
      <div className="boatbuttonContainer"></div>
        ...
        <tbody>
          {filteredBoatTrips.map((boatTrip, index) => (
            <tr key={boatTrip._id} style={{ backgroundColor: index % 2 === 0 ? '#f0f8ff' : '#a7e0a6' }}>
              ...
              <td>
                <button className="boatbtripdeletebt" onClick={() => deleteBoatTrip(boatTrip._id, boatTrip.tripID, boatTrip.boatName)}>
                  <i className="fa-solid fa-trash"></i>
                </button>
                <button className="boatbtripupdatebt">
                  <Link to={`/tripupdate/${boatTrip._id}`} className="boatbtripupdatebt">
